// Grammar for javascript literals
// Extracted from https://github.com/pegjs/pegjs/blob/master/examples/javascript.pegjs
// and adapted for ES 2017 + BigInt literals

{

    var COMMA = {};

    function makeArray(xs) {
        // xs = [ value, COMMA, value, COMMA, COMMA...]
    ...
    function makeObject(xs) {
        // xs = [ pair, [  [stuff,pair], [stuff,pair]... ] ]

        var res = {}, head = xs[0], tail = xs[1];

        res[head[0]] = head[1];

        for (var i = 0; i < tail.length; i++) {
            var pair = tail[i][tail[i].length - 1];
            res[pair[0]] = pair[1];
        }
...
// GB: added binary, octal, bigint and signed
...
NumericLiteral
    = HexIntegerLiteral    { return Number(text()) }
    / BinaryIntegerLiteral { return Number(text()) }
    / OctalIntegerLiteral  { return Number(text()) }
    / DecimalLiteral       { return Number(text()) }
...
BigIntLiteral
    = n:$DecimalIntegerLiteral "n" { return BigInt(n) }

SignedBigIntLiteral
    = "+" n:BigIntLiteral { return n }
    / "-" n:BigIntLiteral { return -n }
...
// GB: use `fromCodePoint` for unicode escapes
...
UnicodeEscapeSequence
    = "u" d:$(HexDigit HexDigit HexDigit HexDigit) { return String.fromCodePoint(parseInt(d, 16)) }
    / "u" "{" d:$(HexDigit+) "}" { return String.fromCodePoint(parseInt(d, 16)) }

// ------------------------------------------------------------------------------------------------------------

RegularExpressionLiteral
    = "/" pattern:$RegularExpressionBody "/" flags:$RegularExpressionFlags { return new RegExp(pattern, flags) }
...
// GB: use a simplified grammar here and deal with elisions in makeArray

ArrayLiteral
    = "[" xs:(ArrayLiteralElement*) __ "]" { return makeArray(xs) }

ArrayLiteralElement
    = __ ","  { return COMMA }
    / __ x:Literal  { return x }
...
ObjectLiteral
    = "{" __ "}" { return {} }
    / "{" __ xs:PropertyNameAndValueList __ "}" { return makeObject(xs) }
    / "{" __ xs:PropertyNameAndValueList __ "," __ "}" { return makeObject(xs) }

PropertyNameAndValueList
    = PropertyAssignment (__ "," __ PropertyAssignment)*

PropertyAssignment
    = key:PropertyName __ ":" __ value:Literal { return [key, value] }
